...
export async function initScenes(jobId: string) {
  return request<{ scenes: number; status: string }>(
    `/reels/${jobId}/init-scenes`, { method: "POST" }
  );
}
...
export function adFileUrl(jobId: string, filename: string): string {
  const token =
    typeof window !== "undefined"
      ? (localStorage.getItem("access_token") ?? sessionStorage.getItem("access_token"))
      : null;
  const base = `/api/ads/${jobId}/file/${encodeURIComponent(filename)}`;
